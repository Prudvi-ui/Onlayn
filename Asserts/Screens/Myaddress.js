...
const MyAddressScreen = ({ navigation }) => {
    ...
    const handleEditAddress = (address) => {
        const [apartment = '', building = ''] = (address.line1 || '')
            .split(',')
            .map((s) => s.trim());
    ...
    const AddressCard = ({ item }) => (
        <View style={styles.card}>
            <TouchableOpacity
                onPress={async () => {
                    await AsyncStorage.setItem('defaultAddress', JSON.stringify(item));
            ...
            >
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    ...
                    <View style={{ flexDirection: 'row' }}>
                        ...
                        >
                            <Icon name="pencil" size={22} color="navy" />
                        </TouchableOpacity>
                        <TouchableOpacity
                ...
                { text: "Cancel", style: "cancel" },
                {
                    ...
                    onPress: async () => {
                        try {
                            ...
                            const data = await response.json();
                            if (response.ok) {
                                Alert.alert('Success', data.message || 'Address deleted');
                                setCustomers((prev) => {
                                ...
                                Alert.alert('Error', data.error || 'Could not delete address');
                            }
                        } catch (error) {
    ...
    const submitAddressToBackend = async () => {
        ...
        if (!isPincodeValid) {
            Toast.show({ type: "error", text1: "Validation", text2: "Invalid pincode" });
            return;
        }
        ...
        try {
            ...
            const addressObj = {
                _id: editingAddressId || undefined, // send _id if editing
                label: selectedType,
            ...
            const res = await fetch(`https://apis.toyshack.in/App/customers/customer-update`, {
                method: "PUT",
            ...
            if (res.ok) {
                Toast.show({
                    type: "success",
                    text1: editingAddressId ? "Address updated" : "Address added",
                });
                        ...
                        >
                            ...
                            <View style={styles.row}>
                                <TextInput
                                    ...
                                    onChangeText={(v) => handleInputChange('apartment', v)}
                                />
                                <TextInput
                            ...
                            >
                                <TouchableOpacity onPress={submitAddressToBackend}>
